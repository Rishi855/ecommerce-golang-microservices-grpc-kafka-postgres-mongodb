...
func (h *ProductHandler) CreateProduct(ctx *gin.Context){
	...
	res, err := h.Client.ProductService.CreateProduct(context.Background(),&proto.CreateProductRequest{
		Name : req.Name,
		Price: req.Price,
		Stock: req.Stock,
	})
	...
	ctx.JSON(http.StatusOK,&proto.CreateProductResponse{
		Id: res.Id,
...
func (h *ProductHandler) FindOne(ctx *gin.Context){
	...
	res, err := h.Client.ProductService.FindOne(ctx,&proto.FindOneRequest{
		Id : id,
	})
	...
	ctx.JSON(http.StatusOK,&proto.FindOneResponse{
		Data: res.Data,
	})
}

func (h *ProductHandler) FindAll(ctx *gin.Context){
	products, err := h.Client.ProductService.FindAll(ctx, &proto.FindAllRequest{})
	if err!=nil{
		ctx.AbortWithError(http.StatusBadGateway,err)
		return
	}
